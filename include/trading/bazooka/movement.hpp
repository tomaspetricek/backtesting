...
namespace trading::bazooka {
    ...
    public:
        ...
        {
            return std::get<index::tag>(data_);
        }

        void tag(const indicator_tag& tag)
        {
            data_.template emplace<to_underlying(index::tag)>(tag);
        }

        std::size_t period() const
        {
            return std::get<index::period>(data_);
        }

        void period(std::size_t period)
        {
            data_. template emplace<to_underlying(index::period)>(period);
        }

        const std::array<fraction_t, n_levels>& levels() const
        {
            return std::get<index::levels>(data_);
        }

        void levels(const std::array<fraction_t, n_levels>& levels)
        {
            data_.template emplace<to_underlying(index::levels)>(levels);
        }

        const std::array<fraction_t, n_levels>& open_sizes() const
        {
            return std::get<index::open_sizes>(data_);
        }

        void open_sizes(const std::array<fraction_t, n_levels>& sizes)
        {
            data_.template emplace<to_underlying(index::open_sizes)>(sizes);
        }
    };
}
