...
BOOST_AUTO_TEST_SUITE(sizer_test)
    ...
    {
        // sum is less than 1
        BOOST_REQUIRE_THROW(order_sizer(std::array<fraction_t, 2>{{{2, 4}, {1, 4}}}), std::invalid_argument);

        // sum is more than 1
        BOOST_REQUIRE_THROW(order_sizer(std::array<fraction_t, 2>{{{5, 10}, {7, 10}}}), std::invalid_argument);
    }

    BOOST_AUTO_TEST_CASE(constructor_test) {
        order_sizer(std::array<fraction_t, 4>{{{4, 8}, {2, 8}, {1, 8}, {1, 8}}});
        order_sizer(std::array<fraction_t, 4>{{{1, 8}, {2, 8}, {2, 8}, {3, 8}}});
        order_sizer(std::array<fraction_t, 1>{{{1}}});
        order_sizer(std::array<fraction_t, 5>{{{1, 10}, {2, 10}, {1, 10}, {4, 10}, {2, 10}}});
        order_sizer(std::array<fraction_t, 4>{{{2, 8}, {4, 8}, {1, 8}, {1, 8}}});
    }
