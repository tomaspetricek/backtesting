...
BOOST_AUTO_TEST_SUITE(position_test)
    BOOST_AUTO_TEST_CASE(spot_test)
    {
        ...
        pos = trading::position{order(amount_t{value_of(open)*0.03307}, open, ptime())};
        open = price_t{44'112.07};
        pos.increase(order(amount_t{value_of(open)*0.03307}, open, ptime()));
        ...
        pos = trading::position{trading::order(amount_t{value_of(open)*0.17046}, open, ptime())};
        open = price_t{6'629.53};
        pos.increase(trading::order(amount_t{value_of(open)*0.17046}, open, ptime()));
        open = price_t{6'261.22};
        pos.increase(trading::order(amount_t{value_of(open)*0.34092}, open, ptime()));
        open = price_t{5'892.91};
        pos.increase(trading::order(amount_t{value_of(open)*0.68185}, open, ptime()));
