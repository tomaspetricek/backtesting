...
BOOST_AUTO_TEST_SUITE(position_test)
    BOOST_AUTO_TEST_CASE(spot_test)
    {
        ...
        trading::position pos{order(1159.0478454228034, open, ptime())};
        pos.close(order(pos.size(), 9'693.60, ptime()));
        ...
        pos.close(order(pos.size(), 46'656.07, ptime()));
        ...
        pos.close(trading::order(pos.size(), 6'074.48, ptime()));
        BOOST_REQUIRE_CLOSE(pos.total_realized_profit<amount>(), -166.85, 1);
    }

    BOOST_AUTO_TEST_CASE(spot_loss_test)
    {
        trading::position pos{trading::order(100, 10'000, ptime())};
        pos.close(trading::order(pos.size(), 5'000, ptime()));
