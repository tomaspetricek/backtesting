...
%---------------------------------------------------------------
\setcounter{page}{1}
Lze obchodovat různá aktiva, jako jsou akcie, kryptoměny nebo indexy, nicméně hlavní cíl zůstává stejný, využít změny tržních cen k maximalizaci zisku a zároveň minimalizovat riziko ztráty.
Pro dosažení konzistentních zisků systematickým způsobem bylo vyvinuto algoritmické obchodování, které využívá počítačově naprogramované strategie pro dělání obchodních rozhodnutí na základě jasně definovaných podmínek.
Nejsou svázáni stejnými omezeními jako lidští obchodníci, necítí únavu a nenechají se svést emocemi.
...
Zároveň je těžší je vysvětlit než algoritmus hrubé síly.
...
\textit{Kombinatorické optimalizační problémy} jsou definované pomocí diskrétních konfiguarčních proměnných.
...
Pravděpodobnost přijetí horšího stavu se s snižuje s $T_{curr}$, nicméně je vždy nenulová \cite{rutenbar, kirkpatrik}.
...
Všechny generují konečnou sekvenci klesajících teplot, liší se však časem potřebným pro jejich výpočet a samotný průběhem.
...
Křížení se používá s cílem vytvořit nové potomky spojením genů 2 nebo více rodičů.
...
\While{$\lnot terminate()$}
    ...
    \For{$n \gets 1$ to $N$} \Comment{Explore neighborhood}
        ...
        \If{$\lnot is\_tabu(S_{cand})$}
            \If{$is\_better(S_{cand}, S_{curr}) \lor aspire()$}
                \State $S_{curr} \gets S_{cand}$
...
Lze ji snadno vizualizovat jako tabulku se 2 sloupci, 1. s nákupními objednávkami a 2. s prodejními objednávkami.
Nákupní objednávky jsou řazeny sestupně, takže objednávka s nejvyšší cenou je 1., jelikož cílem prodejce je prodat za co nejvyšší cenu.
...
Vstupní strategie mohou být jednoduché, jako je překřížení hodnot 2 technických ukazatelů nebo odchýlení se od férové ceny.
...
Důsledkem je, že strategie funguje velmi dobře na historických datech, na kterých byla optimalizována, ale podstatě hůře ve zkutečném obchodování \cite{pardo}.
...
Platforma nabízí 2 druhy optimalizace, hrubou sílu a genetický algoritmus.
...
Ve výřezu jsou vidět 2 ziskové pozice, kde se 1. pozice skládá ze 2 otevřených objednávek a 2. ze 3.
Historická data jsou 1 minutové svíčky, které jsou také vyneseny do grafu.
...
Existují 3 běžně používané ukazatele: jednoduchý klouzavý průměr (SMA), exponenciální klouzavý průměr (EMA) a vážený klouzavý průměr (WMA).
Jsou počítány za určitou periodu $k>0$, které určuje počet vzorků použitých k výpočtu.
...
Tímto způsobem není nutné aktivně čekat, jelikož pozorovatel a pozorovaný jsou časově synchronizovány, musí pouze splňovat dohodnuté rozhraní \cite{observer}.
...
\section{Bazooka}
Bazooka obchodní strategie byla rozdělena do 2 tříd: \texttt{bazooka::strategy} a \texttt{bazooka::manager} za účelem oddělení zájmů.
...
Vyžaduje 2 argumenty, 1. je volatelný objekt a 2. instance třídy \texttt{std::variant}.
...
Třída má pouze 1 členský atribut \texttt{data\_} typu \texttt{std::variant<sma, ema>}.
...
Otevírací objednávky jsou reprezentovány strukturou \texttt{open\_order}, která obsahuje 3 atributy: prodané množství, cenu, za kterou bylo aktivum nakoupeno a čas jejího vytvoření.
...
Podporuje pouze objednávky typu long a 1 uživatelský účet.
...
Pokud je například doba převzorkování 30 a průměrovací metoda \textit{OHLC4}, tak jsou minutové svíčky nejprve převzorkovány na 30minutové svíčky a poté zprůměrovány pomocí \texttt{candle::ohlc4}.

Proces převzorkování probíhá tak, že za dané období se otevírací cena 1. svíčky stane otevírací cenou převzorkované svíčky a uzavírací cena poslední svíčky se stane její uzavírací cenou.
...
Tím to způsobem jsou sledovány čistá hodnota a zůstatek po uzavření pozice.
...
Pro optimalizaci byly vybrány 4 konfigurační proměnné: typ a perioda ukazatele, úrovně a velikosti nákupů.
...
Typ ukazatele je reprezentován enumerační třídou \texttt{indicator\_tag} se 2 možnými hodnotami: \texttt{indicator\_tag::ema} a \texttt{indicator\_tag::sma}.
...
Maximální současný čitatel má buď hodnotu maximálního čitatele, nebo součet zbývajících čitatelů snížený o 1.
...
Pro sledování průběhu heuristických optimalizací se používají objekty pozorovatele, které jsou upozorňovány na události, ke kterým při optimalizaci dochází.
...
Všechny optimalizátory používají objektivní funkci, která nejprve přijme konfiguraci, spustí historickou simulaci, během níž se shromažďují statistiky výkonu, vypočítá hodnotu optimalizačního kritéria a vrátí stav (viz \ref{lst:objective:function}).
...
Náhodně je vždy změněn pouze 1 konfigurační parametr.

Pro určení počtu prohledávaných sousedů byly implementovány 2 algoritmy.
...
\begin{figure}[!htbp]
\centerline{\includegraphics[scale=0.65]{img/simulated-annealing-white-box-progress.png}}
