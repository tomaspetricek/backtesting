...
Data jsou většinou k dispozici jako svíčky ve formátu \textit{OHLCV}, což je zkratka pro otevírací (open), nejvyšší (high), nejnižší (low) a uzavírací (close) ceny a objem (volume).
Nejčastěji jsou ukládány do souborů ve formátu CSV.
Jsou to stejná data, která se používají k vytváření svíčkových grafů.
Počítají se pro různé časové intervaly od minut po dny.

Svíčky se počítají na základě jednotlivých uskutečněných obchodů, které lze také uložit a použít pro přesnější simulaci.
Tento druh dat se nazývá obchodní tiky (trade ticks).
...
Jsou počítány za určité období $k>1$, které určuje počet vzorků použitých k výpočtu.
