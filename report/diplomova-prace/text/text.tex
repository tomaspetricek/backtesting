...
Pseudokód algoritmu lze vidět na \ref{alg:simulated:annealing}, kde $T$ označuje teplotu a $S$ stav.
...
Všechny používají počáteční teplotu $T_{init}$ k výpočtu následující aktuální teploty $T_{curr}$ a hodnotu aktuální iterace $i\geq 0$.
...
Pseudokód algoritmu lze vidět na \ref{alg:genetic:algorithm}, kde $P$ označuje populaci.
...
\begin{algorithmic}
\State $P_{whole} \gets P_{init}$
\While{$\lnot terminate()$}
    \State $P_{parents} \gets select(P_{whole})$
    \State $P_{children} \gets crossover(P_{parents})$
    \State $P_{children} \gets mutate(P_{children})$
    \State $P_{whole} \gets replace(P_{parents}, P_{children})$
\EndWhile
\State \Return {$P_{whole}$}
\end{algorithmic}
\end{algorithm}
...
Pseudokód algoritmu lze vidět na \ref{alg:tabu:search}, kde $S$ označuje stav, $M$ tah a $N$ velikost sousedství.
