...
Formálně lze problém zapsat jako $P=(S, f, \Omega)$, kde $P$ je optimalizační problém, $S$ je vyhledávací prostor, $\Omega$ je sada optimalizačních omezení.
Objektivní funkce je také označována jako cenová nebo fitness funkce.
Funkce slouží k hodnocení konfigurace a její hodnota slouží k jejich porovnání.
...
Tyto problémy obvykle spadají do kategorie \textit{NP-těžkých} problémů, což znamená, že neexistuje žádná deterministická metoda pro jejich řešení v polynomiálním čase \cite{peres}.
...
Obtížnost problému může být měřena pomocí \textit{výpočetní složitosti}, která se vztahuje k době, kterou algoritmus potřebuje k získání výsledku v závislosti na velikosti vstupu.
Je nezávislá na hardwaru a umožňuje srovnání algoritmů. 
Problémy lze podle složitosti rozdělit do tříd.
Rozhodovací problémy, které lze vyřešit v polynomiálním čase na deterministickém Turingově stroji, jsou řazeny do třídy $P$.
Třída $NP$ (nedeterministicky polynomiální) se skládá z rozhodovacích problémů, které lze řešit na nedeterministickém Turingově stroji v polynomiálním čase.
Kontrola certifikátu je ověřitelná v polynomiálním čase na deterministickém Turingově stroji \cite{peres}.

\section{Heuristické metody}
% Definovat kombinatorickou optimalizaci
Účelem heuristik je najít přijatelné řešení v rozumném čase, nicméně nezaručují nalezení optimálního řešení \cite{rutenbar}.

Lze je dělit na heuristiky typu rozděl a panuj a iterativní zlepšování.
Strategie rozděl a panuj nejprve rozdělí problém na menší podproblémy, které jsou vyřešeny samostatně a následně dány dohromady.
Techniky iterativního zlepšování začínají nějakou počáteční konfigurací a během každé iterace ji přeskupují s cílem najít lepší.
Mohou se zastavit po daném počtu iterací nebo když se přestane zlepšovat \cite{kirkpatrik}.

Metaheuristické algoritmy lze klasifikovat do 2 kategorií na algoritmy založené na jediném řešení nebo na populaci.
...
Patří mezi ně: tabu prohledávání, simulované ochlazování nebo řízené místní vyhledávání.
...
Heuristika je založena na fyzickém žíháním pevné látky.
Účelem žíhání materiálu je donutit jej se dostat do nízkoenergetického stavu, jako je krystal.
...
Algoritmus začíná s nějakou počáteční konfigurací a snaží se ji postupně vylepšit.
...
K výběru se používá hodnota fitness funkce.
Vybraní jedinci jsou pak spárováni a kříženi.
Na výsledné potomky může být aplikován operátor mutace.
Jako poslední dojde k nahrazení současné populace \cite{katoch}.
...
Při použití ruletového výběru jsou nejprve jedinci z dané populace namapování na kolo rulety.
...
Každý jedince může být vybrán vícekrát \cite{katoch}.
