...
Zdatnost každého jedince představuje váhu, čím vyšší je váha, tím pravděpodobněji bude daný jedinec vybrán jako rodič.
...
Třída \texttt{en\_block\_replacement} zachovává pouze děti k vytvoření nové generace.
Na druhou stranu třída \texttt{elitism\_replacement} k nim přidá ještě $er\cdot n$ nejzdatnějších rodičů, kde $er \in [0, 1]$ je poměr elitních rodičů a $n$ jejich celkový počet.
...
Má čtyři třídní atributy počet iterací, seznam tabu, aktuální a nejlepší stav.
...
Hodnoty jsou vkládány do paměti pomocí metody \texttt{remember} a zůstanou v ni po určitý počet iterací určeným tabu lhůtou $t>0$, kterou přebírá paměť při konstrukci.
Počet iterací se sníží o jednu pomocí metody \texttt{forget}.
...
K vytvoření haldy používá funkci \texttt{std::push\_heap} a jakmile je plná, použije po vložení nového prvku funkci \texttt{std::pop\_back} k odstranění nejhoršího prvku.
