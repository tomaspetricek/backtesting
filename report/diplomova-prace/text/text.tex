        ...
        for (const auto& tag: tags)
            for (const auto& levels: levels_gen())
                for (const auto& sizes: sizes_gen())
                    co_yield config_type{tag, period, levels, sizes};
};
\end{lstlisting}
...
bool contains(const move_type& move) const
{
    ...
    default:
        return sizes_mem_.contains(move.sizes());
    }
}
\end{lstlisting}
...
Pro generování stavového prostoru mohly být využity generátory  \texttt{systematic::int\_range} a \texttt{random::int\_range}.
