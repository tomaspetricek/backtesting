...
Tyto problémy obvykle spadají do kategorie \textit{NP-těžkých} problémů.
...
Polynomiální algoritmus pro jeden NP-úplný problém by znamenal polynomiální algoritmus pro všechny NP-úplné problémy.
\cite{erickson}.
...
Obchod se provádí plněním nákupních a prodejních objednávek, které se skládají z ceny a množství.
Jsou spárovány pomocí párovacího enginu, který si vede centralizovanou knihu objednávek (order book).
...
Po úspěšném spárování objednávek je provedena výměna \cite{nuti}.

Existují různé varianty knihy objednávek a jsou závislé na dané burze.
...
Mezi aktiva, se kterými lze obchodovat, patří akcie, dluhopisy, opce, komodity, měnové páry, kryptoměny a deriváty.
\cite{nuti}.
